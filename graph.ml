...
module MatrixGraph : Graph = struct
  ...
  let disconnect_nodes (g: 'a graph) (ns: node list) : 'a graph =
    let g = copy g in
    let edges = ns |> List.map (outgoing g) |> List.flatten in
    let rec helper (g': 'a graph) (es: edge list) : 'a graph = match es with
      | [] -> g'
      | x::xs ->
        let g' = disconnect g' x in helper g' xs
    in helper g edges
end
